      ...
      <div className="modal-content" style={{ maxWidth: '800px', width: '92%' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem' }}>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--primary-light)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {t('mlInspectorTitle', 'Soil Moisture ML Model Performance & Inspector')}
          </h2>
        ...
        <div style={{ background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)', padding: '0.75rem 1rem', borderRadius: '8px', marginBottom: '1.25rem', display: 'flex', gap: '0.6rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: '#93c5fd', lineHeight: '1.4' }}>
            <strong style={{ color: '#ffffff' }}>{t('mlMethodologyTitle', 'Soil Moisture Estimation Methodology:')}</strong> {t('mlMethodologyDesc', 'Output values represent "Estimated Soil Moisture" predicted by machine learning models trained on physical soil water balance dynamics. They are not direct hardware sensor readings.')}
          </div>
        </div>
        ...
        <div className="grid-2" style={{ gap: '1rem', marginBottom: '1.25rem' }}>
          <div className="glass-card" style={{ background: 'rgba(0,0,0,0.3)', padding: '0.9rem 1.1rem' }}>
            <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.4rem' }}>
              <Award size={14} color="var(--primary-glow)" /> {t('selectedProdModel', 'Selected Production Model')}
            </div>
            ...
            <div style={{ fontSize: '0.78rem', color: 'var(--primary-light)', marginTop: '0.2rem' }}>
              {t('selectedOverRf', 'Selected over RandomForestRegressor via 80/20 test split validation.')}
            </div>
          </div>

          <div className="glass-card" style={{ background: 'rgba(0,0,0,0.3)', padding: '0.9rem 1.1rem' }}>
            <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.4rem' }}>
              <Database size={14} color="var(--water-cyan)" /> {t('datasetProvenanceTitle', 'Training Dataset Provenance')}
            </div>
            <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#ffffff' }}>
              {modelInfo?.dataset_label || t('simulatedAgriData', 'Simulated/Synthetic Agricultural Training Data')}
            </div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
              {t('datasetProvenanceDesc', 'FAO-56 Soil Water Balance simulation dataset (3,000 samples).')}
            </div>
          </div>
        </div>
        ...
        <h4 style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--primary-glow)', marginBottom: '0.75rem' }}>
          {t('modelCompValidation', 'Model Comparison & Validation Metrics')}
        </h4>
        <div className="grid-2" style={{ gap: '1rem', marginBottom: '1.25rem' }}>
          {/* Gradient Boosting */}
          <div className="glass-card" style={{ border: '1px solid var(--primary-glow)', background: 'rgba(82, 183, 136, 0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
              <span style={{ fontWeight: 800, color: '#ffffff', fontSize: '0.92rem' }}>{t('gradientBoosting', 'Gradient Boosting Regressor')}</span>
              <span className="badge badge-low" style={{ background: 'var(--primary-emerald)', color: '#fff' }}>{t('productionActive', 'Production Active')}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.4rem' }}>
              <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.5rem', borderRadius: '6px', textAlign: 'center' }}>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{t('r2Score', 'R² Score')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--primary-light)' }}>
                  {gbMetrics.r2.toFixed(4)}
                </div>
              </div>
              <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.5rem', borderRadius: '6px', textAlign: 'center' }}>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{t('maeScore', 'MAE (%)')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--water-cyan)' }}>
                  {gbMetrics.mae.toFixed(3)}%
                </div>
              </div>
              <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.5rem', borderRadius: '6px', textAlign: 'center' }}>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{t('rmseScore', 'RMSE (%)')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#ffffff' }}>
          ...
          <div className="glass-card" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-color)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
              <span style={{ fontWeight: 800, color: 'var(--text-muted)', fontSize: '0.92rem' }}>{t('randomForest', 'Random Forest Regressor')}</span>
              <span className="badge" style={{ background: 'rgba(255,255,255,0.1)', color: 'var(--text-muted)' }}>{t('evaluatedBaseline', 'Evaluated Baseline')}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.4rem' }}>
        ...
        <h4 style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--water-cyan)', marginBottom: '0.75rem' }}>
          {t('topFeatureImportance', 'Top Feature Weight Importance Breakdown')}
        </h4>
        ...
        <div style={{ background: 'rgba(82, 183, 136, 0.1)', border: '1px solid var(--border-color)', padding: '0.9rem 1.1rem', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#ffffff' }}>{t('modelRetrainPipeline', 'Model Retraining Pipeline')}</div>
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{t('modelRetrainPipelineDesc', 'Re-generate dataset and retrain Scikit-Learn models on updated soil physics.')}</p>
          </div>
          <button className="btn btn-primary btn-sm" onClick={handleTriggerRetrain} disabled={retraining}>
            ...
            {retraining ? t('retrainingModels', 'Retraining Models...') : t('triggerRetrainBtn', 'Trigger Retrain Pipeline')}
          </button>
        </div>

        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1.25rem' }}>
          <button className="btn btn-outline" onClick={onClose}>{t('closeInspector', 'Close Inspector')}</button>
        </div>
      </div>
    </div>
  );
