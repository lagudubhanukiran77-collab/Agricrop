...
export default function DecisionCard({ analysisResult, fieldName }) {
  if (!analysisResult) {
    return (
      <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>
        ...
        <h3 style={{ fontSize: '1.1rem', color: '#fff', fontWeight: 700 }}>Select a field or tweak environmental parameters</h3>
        <p style={{ fontSize: '0.85rem', marginTop: '4px' }}>AgriCrop Hybrid Intelligence Engine will calculate Estimated Soil Moisture, irrigation decision, water volume & explainable reasoning.</p>
      </div>
    );
  }
  ...
  const getDecisionBadge = (d) => {
    switch (d) {
      case 'Irrigate Now':
        return <span className="badge badge-now" style={{ fontSize: '0.9rem', padding: '0.35rem 0.85rem' }}>💧 Irrigate Now</span>;
      case 'Irrigate Later':
        return <span className="badge badge-later" style={{ fontSize: '0.9rem', padding: '0.35rem 0.85rem' }}>⏳ Irrigate Later</span>;
      case 'Do Not Irrigate':
        return <span className="badge badge-no" style={{ fontSize: '0.9rem', padding: '0.35rem 0.85rem' }}>🍃 Do Not Irrigate</span>;
      default:
        return <span className="badge badge-monitor" style={{ fontSize: '0.9rem', padding: '0.35rem 0.85rem' }}>🔍 Monitor Field</span>;
    }
  };
    ...
    <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--primary-glow)', fontWeight: 700 }}>
              Hybrid Intelligence Decision • {fieldName || 'Selected Field'}
            </span>
            <span style={{ fontSize: '0.7rem', padding: '2px 6px', background: 'rgba(16, 185, 129, 0.15)', color: '#34d399', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '4px', fontWeight: 600 }}>
              🎯 {analysisResult.confidence_score || 85}% AI Confidence
            </span>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '0.72rem', color: 'var(--primary-glow)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>
            Estimated Soil Moisture (AI)
          </div>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#ffffff' }}>
            {estimated_soil_moisture !== undefined ? estimated_soil_moisture : '24.6'}%
          </div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
            Stress Index: <strong style={{ color: stress_score > 60 ? 'var(--status-critical)' : 'var(--primary-light)' }}>{stress_score}/100</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid-3" style={{ background: 'rgba(0, 0, 0, 0.25)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
        <div>
          <div style={{ fontSize: '0.75rem', color: 'var(--water-cyan)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Droplet size={14} /> Recommended Water Volume
          </div>
          <div style={{ fontSize: '1.35rem', fontWeight: 800, color: '#ffffff', marginTop: '2px' }}>
            {estimated_water_liters.toLocaleString()} <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 500 }}>Liters</span>
          </div>
          <div style={{ fontSize: '0.72rem', color: 'var(--primary-glow)' }}>
            ≈ {(estimated_water_liters / 1000.0).toFixed(2)} m³ depth
          </div>
        </div>

        <div>
          <div style={{ fontSize: '0.75rem', color: 'var(--primary-light)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Clock size={14} /> Recommended Duration
          </div>
          {duration_status_msg ? (
            ...
            <>
              <div style={{ fontSize: '1.35rem', fontWeight: 800, color: '#ffffff', marginTop: '2px' }}>
                {recommended_duration_mins} <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 500 }}>mins</span>
              </div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                Optimized for drip irrigation
              </div>
            </>
          )}
        </div>
        ...
        <div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Zap size={14} color="var(--status-medium)" /> Recommended Window
          </div>
      ...
      <div style={{ background: 'rgba(45, 106, 79, 0.12)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '1.1rem' }}>
        <h4 style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--primary-glow)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.85rem' }}>
          <BrainCircuit size={18} /> Why AI Recommends This Decision
        </h4>
        ...
        <div style={{ marginTop: '0.75rem', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.08)', fontSize: '0.72rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
          ℹ️ Estimated Soil Moisture is AI-predicted using weather, crop physics, and irrigation history — not a direct sensor measurement.
        </div>
      </div>
      ...
      <div>
        <h4 style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.6rem' }}>
          <BarChart2 size={15} /> ML Soil Moisture & Decision Weight Breakdown
        </h4>
